<template>
  <q-dialog v-model="isOpenF" @hide="closeModal" persistent>
    <q-card class="w30 q-mt-sm">
      <q-card-section>
        <q-icon name="close" size="2rem" class="float-right cursor-pointer" @click="closeModal" />
        <q-tab-panels v-model="activeTab" animated class="wxl">
          <q-tab-panel name="login" class="q-pt-xs q-pb-xs">
            ...
            <q-form @submit="login">
              ...
              <q-input v-model="loginData.password" label="Contraseña" type="password" required minlength="7" />
              ...
              <q-card-actions>
                <q-btn color="primary" label="Confirmar" type="submit" class="wxl"/>
              </q-card-actions>
            </q-form>
          </q-tab-panel>
          <q-tab-panel name="register" class="q-pt-xs q-pb-xs">
            ...
            <q-form @submit="register">
              <q-input v-model="registerData.username" label="Nombre" required />
              <q-input v-model="registerData.email" label="Correo electrónico" type="email" required />
  ...
  <ErrorPopUp :error="error.message" :open="error.type" @closed="clearError"></ErrorPopUp>
</template>
    ...
    function captchaV3() {
        return new Promise((resolve, reject) => {
            grecaptcha.ready(function () {
                grecaptcha.execute('6Lc099EmAAAAAI_Ra6YRJ35X0FjthvtuXyH0X8E9', {action: 'submit'})
                    .then(function(token){
        ...
        .then(async data => {
          if (data.status == 200){
            ...
            if (data.result.profile != ""){
              ...
              await fetch ('http://127.0.0.1:5000/api/users/profile/'+ userData.value.profile)
    ...
    const login = async(e) => {
        ...
        .then(data => {
            ...
            //Captcha no válido o caducado
                ...
                error.value.type = true;
                error.value.message = "Parece que hay un problema con nuestras bases de datos. Inténtelo de nuevo más tarde";
            }
        }).catch(exception => {
            console.log(exception);
            error.value.type = true;
        ...
        if (registerData.value.password != registerData.value.confirmPassword){
          ...
          .then(data => {
            ...
            if (data.status == 200){
              ...
              error.value.type = true;
              error.value.message = "El nombre de usuario o el correo electrónico ya están en uso";
            }else{
              error.value.type = true;
              error.value.message = "Parece que hay un problema con nuestras bases de datos. Inténtelo de nuevo más tarde";
            }
